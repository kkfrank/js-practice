...
function get_width(start, end) {
    ...
    return globalObj.scale * (to - from) / (1000 * 60);
}
/**
...
//var html_render = render_gantt(data);
    ...
    if (!scrollData) {//第一次进来，定位到中间
        ...
        $scroll.mCustomScrollbar("scrollTo", [scrollData.top, scrollData.left]);
        $scroll.scrollLeft(scrollData.left);
...
function changeScale(el) {
    ...
    if ($(el).hasClass("scale1x")) {//1X
        globalObj.scale = 1;
        $(".timePoint").removeClass().addClass("timePoint scale1x no-select");
...
$(function(){
    ...
    $(".hour-split").on("click",".hour-lf,.hour-ct,.hour-rf",function(event){//切换坐标点击事件
        var $span=$(this).siblings(".probar");
        if($(this).hasClass("hour-lf")){//1X
            if($span.hasClass("probar scale1x")){
                return;
            }else{
                $span[0].className="probar scale1x";
            }
        }else if($(this).hasClass("hour-ct")){//2X
    ...
    $("#infos_details").on("contextmenu", ".plan,.actual", function (event) {
        ...
        //    $planMenu.find("li[data-app=toApproved]").hide();
        event.preventDefault();
...
//      			if(dataFlightInfo.isApproved == 0){
//      				$planMenu.find("li").removeClass("noborder");
//          			$planMenu.find("li:last").addClass("noborder");
//      				$planMenu.find("li[data-app=toApproved]").show();
//      			}
